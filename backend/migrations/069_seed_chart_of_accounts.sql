-- Migration: Seed Chart of Accounts (Indian Schedule III Standard)
-- Purpose: Create default chart of accounts template for Indian companies
-- Note: These are template accounts - actual accounts are created per company

-- ============================================
-- FUNCTION: Create default COA for a company
-- ============================================
...
COMMENT ON FUNCTION create_default_chart_of_accounts(UUID, UUID) IS 'Creates default Indian Schedule III chart of accounts for a company';

-- ============================================
-- TRIGGER: Auto-create COA for new companies
-- ============================================

CREATE OR REPLACE FUNCTION auto_create_coa_for_company()
RETURNS TRIGGER AS $$
BEGIN
    -- Only create COA if the company is new
    IF TG_OP = 'INSERT' THEN
        PERFORM create_default_chart_of_accounts(NEW.id, NULL);
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Note: Uncomment below to auto-create COA for new companies
