...
{
    /// <summary>
    /// Validator for CreateVendorPaymentDto
    /// </summary>
    public class CreateVendorPaymentDtoValidator : AbstractValidator<CreateVendorPaymentDto>
    {
        public CreateVendorPaymentDtoValidator()
        {
            RuleFor(x => x.PartyId)
            ...
            RuleForEach(x => x.Allocations).SetValidator(new CreateVendorPaymentAllocationDtoValidator())
    ...
    /// <summary>
    /// Validator for CreateVendorPaymentAllocationDto
    /// </summary>
    public class CreateVendorPaymentAllocationDtoValidator : AbstractValidator<CreateVendorPaymentAllocationDto>
    {
        public CreateVendorPaymentAllocationDtoValidator()
        {
            RuleFor(x => x.AllocatedAmount)
                .GreaterThan(0).WithMessage("Allocated amount must be greater than 0");

            RuleFor(x => x.TdsAllocated)
                .GreaterThanOrEqualTo(0)
                .WithMessage("TDS allocated cannot be negative")
                .When(x => x.TdsAllocated.HasValue);

            RuleFor(x => x.AllocationType)
                .Must(BeValidAllocationType)
                .WithMessage("Invalid allocation type. Must be one of: bill_settlement, advance_adjustment, debit_note, on_account")
                .When(x => !string.IsNullOrEmpty(x.AllocationType));
        }

        private bool BeValidAllocationType(string? type)
        {
            if (string.IsNullOrEmpty(type)) return true;
            var validTypes = new[] { "bill_settlement", "advance_adjustment", "debit_note", "on_account" };
            return validTypes.Contains(type.ToLowerInvariant());
        }
    }
}
